...
function profilListesi(fenomen) {
  ...
  for (let i = 0; i < fenomen.length; i++) {
    yeniFeno.push(fenomen[i].profile); 
...
function fenomenSil(fenomen, indeks) {
  let kopyafenomen = [...fenomen]
  kopyafenomen.splice(indeks, 1);
  return kopyafenomen
}
...
function fenomenEkle(fenomenler, number, profile, followers, posts, platform) {
  let kopyafenomenler = [...fenomenler];
  const yeniNesne = {
    number: number,
    profile: profile,
    followers: followers,
  ...
  kopyafenomenler.push(yeniNesne);
...
function enFenomenler(fenomenler) {
  // let newArray = fenomenler.filter(fenomen => fenomen.followers > 100000000)
  // let newArray2 = newArray.map(fenomens => fenomens.profile);
  // return newArray2
  let fenos = []
  for (let i = 0; i < fenomenler.length; i++) {
    if (fenomenler[i].followers > 100000000) {
      fenos.push(fenomenler[i].profile)
    }  
  }
  return fenos
}
...
function platformaGoreCokGonderiYapanFenomen(fenomenler, platform) {
  ...
  for (let fenomen of fenomenler) {
    if (fenomen.platform === platform && fenomen.posts !== "NA") {
      if (fenomen.posts > maxGonderi) {
        maxGonderi = fenomen.posts;
        enCokGonderiYapan = fenomen.profile;
      }
    }
  }
  return enCokGonderiYapan;
}
